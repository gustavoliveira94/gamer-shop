...
export const Card: React.FC<CardProps> = ({ game }) => {
  return (
    <div className="w-[380px] h-[436px] rounded-2xl border-[0.5px] border-secondary p-[24px]">
      <Image
        src={game.image}
        alt={game.name}
      ...
      <div className="flex flex-col gap-2 mt-4 mb-[20px]">
        ...
        <div className="flex items-center justify-between">
          <p className="text-primary text-lg font-bold" title={game.name}>
            {game.name.length >= 26 ? `${game.name.slice(0, 26)}...` : game.name}
          </p>
          <span className="text-primary text-lg font-bold">${game.price}</span>
        </div>
      </div>
      <button className="w-[332px] h-[56px] border border-cta-primary rounded-lg font-bold text-primary text-base">
        ADD TO CART
      </button>
    </div>
  )
